module.exports = {
    ...
    addPost: async (req, res) => {
        const {title, address, description, url} = req.body;
        const db = req.app.get('db');
        if(req.session.user) {
            const post = await db.posts.addPost(req.session.user.id, title, address, description, url);
    ...
    editPost: async (req, res) => {
        const {spot_id, title, address, description} = req.body;
        const db = req.app.get('db');
        const posts = await db.posts.updatePost(spot_id, title, address, description, req.session.user.id);
